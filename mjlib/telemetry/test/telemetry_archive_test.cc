...
#include "mjlib/telemetry/telemetry_util.h"

using namespace mjlib::telemetry;
...
struct Test1 {
  ...
  SubTest1 value_sub;
  std::optional<SubTest1> value_optional;
  int32_t value_i32 = 6;
...
BOOST_AUTO_TEST_CASE(TelemetryArchiveSchemaTest) {
  ...
  std::string schema_str = archive.schema();
  FastIStringStream istr(schema_str);
  FastOStringStream repr;
...
BOOST_AUTO_TEST_CASE(TelemetryArchiveDataTest) {
  TelemetryWriteArchive<Test1> archive;
  FastIStringStream istr(archive.schema());
  FastOStringStream repr;
  TelemetryReadStream stream(istr);
  ...
  FastIStringStream data_istr(result);
